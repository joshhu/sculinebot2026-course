"""
東吳大學資料系 2026 LINEBOT —— 手機商城 × Supabase × 熱賣輪播 × 個人購物車 × 結帳

完整的迷你電商流程，全部資料放 Supabase（雲端 Postgres），全部操作都用「按鈕」：
  逛（熱賣商品）→ 加入購物車 → 我的購物車（移除／再加一個）→ 結帳 → 我的訂單

「記得不同使用者」靠 LINE 每個事件都帶的 `event.source.user_id`（每人一組固定 ID）。

操作入口（不用打字，全部可點）：
  - 底部 rich menu：🔥熱賣推薦／🛒我的購物車／📦我的訂單／…
  - 商品卡按鈕（postback）：加入購物車、規格詳情
  - 購物車卡按鈕：移除、再加一個
  - quick reply 快速回覆：結帳、清空購物車、繼續逛…（會跟著訊息浮在鍵盤上方）
  文字指令（熱賣商品／我的購物車／清空購物車／結帳／我的訂單）仍可用，但示範用按鈕即可。

三張表、兩種金鑰（重要教學點）：
  - phones（公開商品目錄）：publishable key + RLS 公開讀取，只讀、金鑰可放 bot。
  - cart_items / orders（私有，跟人綁）：server 端 service_role 金鑰（繞過 RLS）後端讀寫。

需要的環境變數：
  LINE_CHANNEL_SECRET / LINE_CHANNEL_ACCESS_TOKEN / SUPABASE_URL /
...
def render_phone_page(p):
    ...
    hot = '<span class="hot">🔥 熱賣商品</span>' if p.get("is_hot") else ""
    inner = (
        ...
        f'<div class="price">NT${p.get("price", 0):,}<small>含稅</small></div>{hot}'
...
def handle_text_message(event):
    ...
    if text in TRIGGERS:
        ...
        reply(event.reply_token, TextMessage(
            text="用下方選單操作 👇 或輸入「熱賣商品」",
            quick_reply=quick(("🔥 熱賣商品", "menu=hot"), ("🛒 我的購物車", "menu=cart"), ("📦 我的訂單", "menu=orders"))))


# 底部 rich menu 尚未實作的占位鍵（data 形如 menu=coupon）
RICH_MENU_LABELS = {"coupon": "領優惠券", "support": "線上客服"}
...
def handle_postback(event):
    ...
    if menu:
        if menu == "hot":
            ...
            label = RICH_MENU_LABELS.get(menu, menu)
            reply(event.reply_token, TextMessage(
                text=f"「{label}」功能示範中，敬請期待 🙌",
                quick_reply=quick(("🔥 熱賣商品", "menu=hot"), ("🛒 我的購物車", "menu=cart"))))
        return

    # 2) 結帳 / 清空 / 選分類（無商品 id 的按鈕）
    if action == "checkout":
        reply(event.reply_token, _need_dm(uid) or msg_checkout(uid))
        return
    if action == "clearcart":
