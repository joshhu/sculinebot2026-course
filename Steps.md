...
### 1. 安裝 uv
1. 開啟 Powershell
2. 輸入 `powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"` 並按下 Enter 鍵
3. 在 Powershell 中輸入 `uv --version` 確認安裝成功

### 2. 安裝 hf
1. 在 Powershell 中
2. 輸入 `powershell -ExecutionPolicy ByPass -c "irm https://hf.co/cli/install.ps1 | iex"` 並按下 Enter 鍵
3. 在 Powershell 中輸入 `hf --version` 確認安裝

### 3. 安裝 gh
1. 開啟 Powershell 中
2. 輸入 `winget install --id GitHub.cli` 並按下 Enter 鍵
3. 在 Powershell 中輸入 `gh --version` 確認安裝

### 4. 安裝 Coding Agent
1. Claude Code：在Powershell中輸入`irm https://claude.ai/install.ps1 | iex`並按下 Enter 鍵
2. Codex ：在Powershell中輸入`winget install Codex -s msstore`並按下 Enter 鍵
